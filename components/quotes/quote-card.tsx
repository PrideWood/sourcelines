...
export function QuoteCard({
  ...
  return (
    <Card
      className={`group relative border-border/80 transition-colors hover:border-foreground/20 ${uniformHeight ? "flex h-[25rem] flex-col overflow-hidden" : ""}`}
    >
      <Link
        aria-label="查看引文详情"
      ...
      <CardContent className={`pointer-events-none relative z-20 ${uniformHeight ? "flex min-h-0 flex-1 flex-col" : ""}`}>
        ...
        <div className={uniformHeight ? "quote-meta-region mt-4 space-y-2" : "mt-4 space-y-2"}>
          <div className={`quote-tag-row flex flex-wrap gap-2 ${uniformHeight ? "" : "min-h-0"}`}>
            {quote.quote_tags && quote.quote_tags.length > 0 ? (
              quote.quote_tags
                .slice(0, 3)
                .map((item, index) => <Badge key={`${quote.id}-${item.tag.name}-${index}`}>{formatTagLabel(item.tag.name)}</Badge>)
            ) : uniformHeight ? (
          ...
          <p className="line-clamp-1 text-xs text-muted-foreground">出处：{quote.source_locator ?? "未提供"}</p>
        </div>
      </CardContent>
    </Card>
  );
